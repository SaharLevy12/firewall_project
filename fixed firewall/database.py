...
class Database:
    ...
    def register_user(self, request):
        ...
        with self.lock:
            self.cursor.execute("INSERT INTO users VALUES (?, ?, ?, ?)", (username, email, password, is_admin))
            self.conn.commit()
        return
    ...
    def check_login(self, request):
        ...
        with self.lock:
            self.cursor.execute("SELECT username, isAdmin FROM users WHERE email=? AND password=?", (email, password))
